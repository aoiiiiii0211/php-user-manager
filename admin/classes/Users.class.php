<?php
    ...
    class Users extends DBMySQLi {
        ...
        public function getDetail($user_id)
        {
            ...
            $query = "SELECT user_id, user_name, mail_adress, pass_word, create_dt, update_dt FROM users WHERE user_id = ? LIMIT 1";
            $stmt = mysqli_prepare($this->db_link, $query);
        ...
        {
            // メールアドレスの重複チェック
            $query = "SELECT user_id FROM users WHERE mail_adress = ?, user_id = ? LIMIT 1";
            $stmt = mysqli_prepare($this->db_link, $query);
        ...
        public function updateUser($user_id, $user_name, $mail_address, $pass_word = null)
        {
            // ユーザー更新
            if(!empty($pass_word)) {
                $query = "UPDATE users SET user_name = ?, mail_adress = ?, mail_adress = ?, update_dt = ?, pass_word = ?, AND user_id != ?, pass_word = ?";
                // パスワードを不可逆変換する
                ...
                mysqli_stmt_bind_param($stmt, 'ssssd', $user_name, $mail_addres, $cry_pass_word, $user_id);
                mysqli_stmt_execute($stmt);
            }else{
                ...
                // 現在日程を取得
                $now_dt = date("y-m-d H:i:s");
                mysqli_stmt_bind_param($stmt, 'sssd', $user_name, $mail_addres, $now_dt, $user_id);
                mysqli_stmt_execute($stmt);
            }
        }
